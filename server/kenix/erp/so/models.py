...
class SoModel(EndpointsModel):
    so_num = ndb.StringProperty()
    ...
    def line_of_items(self):
        ...
        """
        q = SoLineModel.query(ancestor=self.key)
        return q.fetch()


class SoLineModel(EndpointsModel):
    """
    PO Line of Item.
    """
    _parent = None
    ...
    # header info
    so_key = ndb.KeyProperty(SoModel)
    so_num = ndb.StringProperty()
    order_date = ndb.DateProperty()

    # line info
    so_line_num = ndb.IntegerProperty()
    ...
    so_line_status = ndb.StringProperty(choices=['N', 'P', 'B', 'X', 'C'])

    def SetKey(self):
        """
        set key of the SoLineModel object that includes ancestor key from SoModel object
        """
        if self._parent is not None and self._id is not None:
            key = ndb.Key(SoModel, self._parent, SoLineModel, self._id)
    ...
    def ParentSet(self, value):
        if not isinstance(value, basestring):
            raise TypeError('SoModel name must be a string.')
        self._parent = value
        if ndb.Key(SoModel, value).get() is None:
            raise endpoints.NotFoundException('SoModel %s does not exist.' % value)
        self.SetKey()
        self._endpoints_query_info.ancestor = ndb.Key(SoModel, value)
